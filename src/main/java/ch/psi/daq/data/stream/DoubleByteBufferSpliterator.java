 ...
 */
public class DoubleByteBufferSpliterator<T> implements Spliterator.OfDouble {
	 ...
	 *            The ByteConverter
	 */
	public DoubleByteBufferSpliterator(ByteBuffer buffer, ByteValueSpliterer<T> spliterer) {
	 ...
	 *            The characteristics
	 */
	protected DoubleByteBufferSpliterator(ByteBuffer buffer, int origin, int fence, ByteValueSpliterer<T> spliterer, int additionalCharacteristics) {
		this.buffer = buffer;
	...
	public OfDouble trySplit() {
		...
		return (lo >= mid)
				? null
				: new DoubleByteBufferSpliterator<T>(this.buffer, lo, startIndex = mid, this.spliterer, characteristics);
	}
	...
	public void forEachRemaining(DoubleConsumer action) {
		...
		if ((buf = buffer).limit() >= (hi = endIndex) &&
			...
			do {
				action.accept(conv.getAsDouble(buf, i));
	...
	public boolean tryAdvance(DoubleConsumer action) {
		...
		if (startIndex >= 0 && startIndex < endIndex) {
			ByteValueSpliterer<T> spliterer = this.spliterer;
			action.accept(spliterer.getByteValueConverter().getAsDouble(buffer, startIndex));
